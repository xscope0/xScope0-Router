...
</div></body></html>`}function l(){s&&(clearTimeout(s),s=null),a&&(a.close(),a=null)}let u=null,d=null,h=new Map;function p(){d&&(clearTimeout(d),d=null),u&&(u.close(),u=null)}e.s(["clearCodexSession",0,function(e){i.delete(e)},"clearXaiSession",0,function(e){h.delete(e)},"getCodexSessionStatus",0,function(e){return i.get(e)||null},"getXaiSessionStatus",0,function(e){return h.get(e)||null},"registerCodexSession",0,function({state:e,codeVerifier:t,redirectUri:r}){return!!e&&!!t&&!!r&&(i.set(e,{codeVerifier:t,redirectUri:r,status:"pending",createdAt:Date.now()}),!0)},"registerXaiSession",0,function({state:e,codeVerifier:t,redirectUri:r}){return!!e&&!!t&&!!r&&(h.set(e,{codeVerifier:t,redirectUri:r,status:"pending",createdAt:Date.now()}),!0)},"startCodexProxy",0,function(n){return new Promise(u=>{if(a)return void u({success:!0});let d=t.default.createServer(async(t,a)=>{let s=new r.URL(t.url,"http://localhost");if("/callback"!==s.pathname&&"/auth/callback"!==s.pathname){a.writeHead(404),a.end("Not found");return}let o=s.searchParams.get("code"),u=s.searchParams.get("state"),d=s.searchParams.get("error"),h=u?i.get(u):null;if(h){try{if(d)throw Error(s.searchParams.get("error_description")||d);if(!o)throw Error("No authorization code received");let{exchangeTokens:t}=await e.A(633998),{createProviderConnection:r}=await e.A(52307),n=await t("codex",o,h.redirectUri,h.codeVerifier,u),i=await r({provider:"codex",authType:"oauth",...n,expiresAt:n.expiresIn?new Date(Date.now()+1e3*n.expiresIn).toISOString():null,testStatus:"active"});h.status="done",h.connectionId=i.id,h.email=i.email,a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(c(!0,"You can close this window."))}catch(e){h.status="error",h.error=e.message,a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(c(!1,e.message))}finally{l()}return}let p=`http://localhost:${n}/callback${s.search}`;a.writeHead(302,{Location:p}),a.end(),l()});d.listen(o,"127.0.0.1",()=>{a=d,s=setTimeout(()=>l(),3e5),u({success:!0})}),d.on("error",e=>{"EADDRINUSE"===e.code?u({success:!1,reason:"port_busy"}):u({success:!1,reason:e.message})})})},"startLocalServer",0,function(e,n=null){return new Promise((a,s)=>{let o=t.default.createServer((t,n)=>{let a=new r.URL(t.url,"http://localhost");if("/callback"===a.pathname||"/auth/callback"===a.pathname){let t=Object.fromEntries(a.searchParams);n.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),n.end(`<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
</html>`),e(t)}else n.writeHead(404),n.end("Not found")});o.listen(n||0,"127.0.0.1",()=>{let{port:e}=o.address();a({server:o,port:e,close:()=>o.close()})}),o.on("error",e=>{"EADDRINUSE"===e.code&&n?s(Error(`Port ${n} is already in use. Please close other applications using this port.`)):s(e)})})},"startXaiProxy",0,function(n){return new Promise(a=>{if(u)return void a({success:!0});let s=t.default.createServer(async(t,a)=>{let s=new r.URL(t.url,"http://localhost");if("/callback"!==s.pathname&&"/auth/callback"!==s.pathname){a.writeHead(404),a.end("Not found");return}let o=s.searchParams.get("code"),i=s.searchParams.get("state"),l=s.searchParams.get("error"),u=i?h.get(i):null;if(u){try{if(l)throw Error(s.searchParams.get("error_description")||l);if(!o)throw Error("No authorization code received");let{exchangeTokens:t}=await e.A(633998),{createProviderConnection:r}=await e.A(52307),n=await t("xai",o,u.redirectUri,u.codeVerifier,i),d=await r({provider:"xai",authType:"oauth",...n,expiresAt:n.expiresIn?new Date(Date.now()+1e3*n.expiresIn).toISOString():null,testStatus:"active"});u.status="done",u.connectionId=d.id,u.email=d.email,a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(c(!0,"You can close this window."))}catch(e){u.status="error",u.error=e.message,a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(c(!1,e.message))}finally{p()}return}let d=`http://localhost:${n}/callback${s.search}`;a.writeHead(302,{Location:d}),a.end(),p()});s.listen(56121,"127.0.0.1",()=>{u=s,d=setTimeout(()=>p(),3e5),a({success:!0})}),s.on("error",e=>{"EADDRINUSE"===e.code?a({success:!1,reason:"port_busy"}):a({success:!1,reason:e.message})})})},"stopCodexProxy",0,l,"stopXaiProxy",0,p])},52307,e=>{e.v(t=>Promise.all(["server/chunks/src_models_index_1ip7g0d.js"].map(t=>e.l(t))).then(()=>t(756862)))}];

//# sourceMappingURL=%5Broot-of-the-server%5D__0425o8d._.js.map