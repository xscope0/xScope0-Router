...
export default function PiToolCard({ tool, baseUrl, apiKeys, activeProviders = [], cloudEnabled }) {
  ...
  const effectiveBaseUrl = () => {
    const u = (customBaseUrl || baseUrl || "http://localhost:20128").replace(/\/+$/, "");
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div className="flex items-start justify-between gap-3 text-left">
        ...
        {status?.hasVansRoute && <span className="rounded-full bg-green-500/10 px-1.5 py-0.5 text-[10px] font-medium text-green-600">Connected</span>}
      </div>
      ...
      <div className="mt-4 flex flex-col gap-4 border-t border-border pt-4">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[8rem_auto_1fr] sm:items-center">
          ...
          <input value={customBaseUrl || effectiveBaseUrl()} onChange={(e) => setCustomBaseUrl(e.target.value)} className="rounded border border-border bg-surface px-2 py-2 text-xs" />
        </div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[8rem_auto_1fr] sm:items-center">
