...
async function scheduleKiroReactivation(connectionId) {
  ...
  log.info("KIRO_REACTIVATION", "Marked inactive, auto-reactivate in 30min", { connectionId });
  const timeoutId = setTimeout(async () => {
    try {
      await updateProviderConnection(connectionId, { isActive: 1 });
      log.info("KIRO_REACTIVATION", "Auto-reactivated", { connectionId });
