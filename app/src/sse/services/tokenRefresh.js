...
// ─── Kiro auto-reactivation ──────────────────────────────────────────────────

// Track scheduled Kiro provider reactivations
const kiroReactivationSchedule = new Map();
const KIRO_REACTIVATION_MS = 45 * 60 * 1000;

async function scheduleKiroReactivation(connectionId, reason = "refresh_failed") {
  if (kiroReactivationSchedule.has(connectionId)) clearTimeout(kiroReactivationSchedule.get(connectionId));
  await updateProviderConnection(connectionId, { isActive: 0 });
  log.info("KIRO_REACTIVATION", "Marked inactive, auto-reactivate in 45min", { connectionId, reason });
  const timeoutId = setTimeout(async () => {
    try {
      await updateProviderConnection(connectionId, { isActive: 1 });
      log.info("KIRO_REACTIVATION", "Auto-reactivated", { connectionId, reason });
    } catch (err) { log.error("KIRO_REACTIVATION", "Reactivation failed", { connectionId, error: err.message }); }
    kiroReactivationSchedule.delete(connectionId);
  }, KIRO_REACTIVATION_MS);
  kiroReactivationSchedule.set(connectionId, timeoutId);
}
...
export async function checkAndRefreshToken(provider, credentials) {
  ...
  if (_shouldRefreshCredentials(provider, creds)) {
    ...
    } else if (provider === "kiro" && creds.connectionId) {
      // Kiro refresh failed - schedule auto-reactivation
      await scheduleKiroReactivation(creds.connectionId);
    }
  }
