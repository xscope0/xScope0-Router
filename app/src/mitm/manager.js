...
// Copy bundled MITM runtime into DATA_DIR so MITM doesn't lock node_modules
...
function ensureRuntimeServer(bundledPath) {
  try {
    ...
    // Dev mode: use source path directly.
    ...
    // New MITM server has relative requires (./logger, ./config, ./handlers...).
    // Copy the whole runtime directory, not just server.js.
    const stampFile = path.join(runtimeDir, ".runtime-stamp");
    const sourceStamp = fs.readdirSync(bundledDir, { recursive: true }).join("\n") + "\n" + fs.statSync(bundledPath).mtimeMs;
    if (fs.existsSync(runtimeServer) && fs.existsSync(stampFile)) {
      try {
        if (fs.readFileSync(stampFile, "utf8") === sourceStamp) return runtimeServer;
      } catch { /* recopy */ }
    }

    fs.rmSync(runtimeDir, { recursive: true, force: true });
    copyDirRecursive(bundledDir, runtimeDir);

    // dns/dnsConfig.js imports ../../shared/constants/mitmToolHosts.js from
    // the copied runtime location, so mirror that shared constant too.
    const sharedSrc = path.join(process.cwd(), "src", "shared", "constants", "mitmToolHosts.js");
    const sharedDst = path.join(DATA_DIR, "runtime", "shared", "constants", "mitmToolHosts.js");
    if (fs.existsSync(sharedSrc)) {
      fs.mkdirSync(path.dirname(sharedDst), { recursive: true });
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  // Start means "replace with one fresh MITM". Never reuse a PID file here:
  // stale sudo parents / orphaned node children can look healthy but block a
  // new configured instance. Kill known leftovers first to prevent zombies and
  // keep exactly one MITM server.
  if (serverProcess && !serverProcess.killed) {
    log(`♻️ Replacing in-memory MITM process (PID: ${serverProcess.pid})`);
  } else if (fs.existsSync(PID_FILE)) {
    try {
      const savedPid = parseInt(fs.readFileSync(PID_FILE, "utf-8").trim(), 10);
      if (savedPid && isProcessAlive(savedPid)) log(`♻️ Replacing existing MITM process (PID: ${savedPid})`);
    } catch { /* ignore */ }
  }
