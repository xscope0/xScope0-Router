...
export default function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onUpdateProviderData, onEdit, onDelete, showDelete = true, oneByOneStatus = null, autoPing = null, showProxyAutomation = false }) {
  ...
  const rotateDropdownRef = useRef(null);
  ...
  useEffect(() => {
    if (!showProxyDropdown && !showRotateDropdown) return;
    const handler = (e) => {
      if (proxyDropdownRef.current && !proxyDropdownRef.current.contains(e.target)) {
        setShowProxyDropdown(false);
      }
      if (rotateDropdownRef.current && !rotateDropdownRef.current.contains(e.target)) {
        setShowRotateDropdown(false);
      }
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, [showProxyDropdown, showRotateDropdown]);
    ...
    <div className={`group flex min-w-0 flex-col gap-3 rounded-lg p-2 transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between ${connection.isActive === false ? "opacity-60" : ""}`}>
      <div className="flex min-w-0 flex-1 items-start gap-2 sm:items-center sm:gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {showProxyAutomation && connection.providerSpecificData?.autoRotateProxyMinutes && (
              <Badge variant="primary" size="sm">
                rotate {connection.providerSpecificData.autoRotateProxyMinutes}m
              </Badge>
            )}
      ...
      <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
        <div className="grid flex-1 grid-cols-3 gap-1 sm:flex sm:flex-none">
          ...
          {showProxyAutomation && (
            <div className="relative" ref={rotateDropdownRef}>
              <Tooltip text="Rotate proxy every 5/10/15 minutes.">
                <button
                  onClick={() => setShowRotateDropdown((v) => !v)}
                ...
                >
                  ...
                  <span className="text-[10px] leading-tight">{connection.providerSpecificData?.autoRotateProxyMinutes ? `${connection.providerSpecificData.autoRotateProxyMinutes}m` : "Rotate"}</span>
                </button>
              </Tooltip>
              {showRotateDropdown && (
                <div className="absolute right-0 top-full z-50 mt-1 min-w-[130px] rounded-lg border border-border bg-bg py-1 shadow-lg">
                  {[[null, "Off"], [5, "Every 5m"], [10, "Every 10m"], [15, "Every 15m"]].map(([value, label]) => (
                    <button
                      key={label}
                      onClick={() => { onUpdateProviderData?.({ autoRotateProxyMinutes: value }); setShowRotateDropdown(false); }}
                      className={`w-full px-3 py-1.5 text-left text-sm hover:bg-black/5 dark:hover:bg-white/5 ${connection.providerSpecificData?.autoRotateProxyMinutes === value ? "font-medium text-primary" : "text-text-main"}`}
                    >
                      {label}
                    </button>
                  ))}
          ...
          {showProxyAutomation && (
            <Tooltip text="Switch to next active proxy pool when this account gets an upstream error.">
              <button
                onClick={() => onUpdateProviderData?.({ autoRotateProxyOnError: connection.providerSpecificData?.autoRotateProxyOnError !== true })}
              ...
              >
                ...
                <span className="text-[10px] leading-tight">Err-proxy</span>
              </button>
            </Tooltip>
          )}
          {autoPing && (
