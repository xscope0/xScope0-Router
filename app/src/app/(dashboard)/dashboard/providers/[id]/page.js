...
export default function ProviderDetailPage() {
  ...
  const fetchConnections = useCallback(async () => {
    try {
      ...
      setProviderAutoDeactivate(override.autoDeactivateOnError === true);
  ...
  const handleProviderAutoDeactivateToggle = async (enabled) => {
    setProviderAutoDeactivate(enabled);
    try {
      const settingsRes = await fetch("/api/settings", { cache: "no-store" });
      const settingsData = settingsRes.ok ? await settingsRes.json() : {};
      const current = settingsData.providerStrategies || {};
      const override = { ...(current[providerId] || {}) };
      if (enabled) override.autoDeactivateOnError = true;
      else delete override.autoDeactivateOnError;
      const updated = { ...current };
      if (Object.keys(override).length === 0) delete updated[providerId];
      else updated[providerId] = override;
      await fetch("/api/settings", {
      ...
      console.log("Error saving provider error policy:", error);
      setProviderAutoDeactivate(!enabled);
    }
  };
  ...
  const deleteConnectionIds = async (idsToDelete) => {
    let failed = 0;
  ...
  const handleBulkDelete = () => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        setConfirmState(null);
        await deleteConnectionIds([...selectedConnectionIds]);
      }
    });
  };
  ...
  const handleDeleteInactive = () => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        setConfirmState(null);
        await deleteConnectionIds(idsToDelete);
      }
    });
  };
  ...
  const handleUpdateProviderData = async (conn, patch) => {
    ...
    try {
      ...
      console.log("Error updating provider data:", error);
    }
  };
        ...
        <Card>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              ...
              {!isFreeNoAuth && (
                <button
                  onClick={() => handleProviderAutoDeactivateToggle(!providerAutoDeactivate)}
